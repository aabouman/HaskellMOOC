...
sums :: Int -> [Int]
sums n = [sum [j | j <- [1..i]] | i <- [1..n]]
    where sum list = case list of []     -> 0
                                  (x:xs) -> x + sum xs
...
indexDefault (x:xs) i def = case (i, xs) of (0, _)  -> x
                                            (_, []) -> def
                                            (_, _)  -> indexDefault xs (i-1) def
...
sumsOf xs = sumsOf' 0 xs
    where sumsOf' sum xs = case xs of []     -> []
                                      (y:ys) -> (sum + y):(sumsOf' (sum + y) ys)
...
mymaximum bigger initial a = case a of []     -> initial
                                       (x:xs) -> if bigger x initial then mymaximum bigger x xs else mymaximum bigger initial xs

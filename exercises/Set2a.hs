...
palindromify s = if isPalindrome s then s else palindromify (clip s)
    where clip s = drop 1 (take (length(s)-1) s)
...
greet first last = "Hello, " ++ first ++ last_str ++ "!"
    where last_str = case last of Just str -> " "++ str
                                  Nothing  -> ""
...
addEithers a b = addEithers' a b
    where addEithers' (Right a) (Right b) = Right (a + b)

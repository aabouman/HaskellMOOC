...
instance Ord Country where
  compare a b
    | a == b = EQ
    | otherwise = case (a, b) of (_, Switzerland) -> LT
                                 (Finland, _) -> LT
                                 (_, _) -> GT
  (<=) a b = case compare a b of EQ -> True
                                 LT -> True
                                 GT -> False
...
instance Eq Name where
  (==) (Name s1) (Name s2) = (length s1 == length s2) && and (zipWith (\x y -> Data.Char.toLower x == Data.Char.toLower y) s1 s2)
...
instance Ord Number where
  compare a b = case (a, b) of (Infinite, Infinite) -> EQ
                               (_, Infinite) -> LT
                               (Infinite, _) -> GT
                               (Finite c, Finite d) -> compare c d
...
instance Num RationalNumber where
  (RationalNumber a1 b1) + (RationalNumber a2 b2) = simplify (RationalNumber (a1 * b2 + b1 * a2) (b1 * b2))
  (RationalNumber a1 b1) * (RationalNumber a2 b2) = simplify (RationalNumber (a1 * a2) (b1 * b2))
  abs (RationalNumber a1 b1) = RationalNumber (abs a1) (abs b1)

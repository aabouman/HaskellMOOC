...
allEqual [] = True
allEqual [x] = True
allEqual (x:ys) = foldr (\a b -> a == x && b) True ys
...
distinct []     = True
distinct (x:ys) = foldr (\a b -> a /= x && b) True ys && distinct ys
...
-- rangeOf [] = 0
-- rangeOf as = listMax as 0 - listMin as 0
--     where listMax as m = case as of []     -> m
--                                     (a:bs) -> listMax bs (max a m)
--           listMin as m = case as of []     -> m
--                                     (a:bs) -> listMin bs (min a m)
rangeOf :: (Ord a, Num a) => [a] -> a
rangeOf (a:bs) = foldr max a bs - foldr min a bs
...
longest :: Ord a => [[a]] -> [a]
longest [] = []
longest (x:ys) = foldr shorterList x ys
    where shorterList (a:as) (b:bs)
              | length as == length bs = if a < b then a:as else b:bs
              | otherwise              = if length as < length bs then b:bs else a:as
...
incrementKey key = map incrementKey
    where incrementKey (k, v)
              | key == k  = (k, v + 1)
              | otherwise = (k, v)
------------------------------------------------------------------------------
...
winner scores player1 player2 = case (Map.lookup player1 scores, Map.lookup player2 scores) of (Nothing, Just _) -> player2
                                                                                               (Just _, Nothing) -> player1
                                                                                               (Just a, Just b)  -> if a >= b then player1 else player2
...
freqs = foldr (Map.alter alterFunc) Map.empty
    where alterFunc val = case val of Nothing  -> Just 1
                                      (Just v) -> Just (v + 1)
...
transfer from to amount bank
    | amount < 0 = bank
    | otherwise = case (Map.lookup from bank, Map.lookup to bank)
                  of (Nothing, _) -> bank
                     (_, Nothing) -> bank
                     (Just a, Just b) -> if a < amount
                                         then bank
                                         else depositWithdraw to amount (depositWithdraw from (-amount) bank)
    where
      depositWithdraw person amount = Map.alter (\val -> case val of Nothing  -> Nothing
                                                                     (Just v) -> Just (v + amount)) person
...
maxIndex arr = case (assocs arr) of ((i,e):as) -> max as (i,e)
    where max ((i,e):as) (mi,me)
              | null as   = if e > me then i else mi
              | e > me    = max as (i,e)
              | otherwise = max as (mi,me)

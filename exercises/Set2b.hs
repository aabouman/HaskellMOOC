...
leftpad str len
    | length(str) < len = leftpad (" "++str) len
    | otherwise         = str
...
countdown cnt = "Ready! " ++ countdown' cnt
    where countdown' cnt
              | cnt <= 0 = "Liftoff!"
              | otherwise = (show cnt) ++ "... " ++ countdown' (cnt - 1)
...
-- smallestDivisor n = smallestDivisor' n (n `div` 2)
--     where smallestDivisor' x y
--               | y <= 1           = 1
--               | (x `mod` y) == 0 = y

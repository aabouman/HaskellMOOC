...
mapMaybe f x = case x of Nothing -> Nothing
                         Just x  -> Just (f x)
...
mapMaybe2 f x y = case (x, y) of (Nothing, _) -> Nothing
                                 (_, Nothing) -> Nothing
...
powers k max
    | k < 2     = [-1]
    | otherwise = reverse (powers' 0 [])
    where powers' pow pows
              | k^pow <= max = powers' (pow+1) ((k^pow):pows)
              | otherwise   = pows
...
joinToLength len strs = [(first ++ last) | first <- strs, last <- strs, (length (first ++ last)) == len]
...
sumRights list = sum (map (\(x) -> case x of Right a -> a
                                             Left _ -> 0) list)
...
multiCompose funs x = case funs of []   -> x
                                   f:fs -> f (multiCompose fs x)
...
multiApp f gs x = f (listApply gs)
    where listApply funs = case funs of []   -> []
                                        f:fs -> (f x):(listApply fs)
...
interpreter commands = interpret commands 0 0
    where interpret [] x y = []
          interpret (command:commands) x y = case command of "up"     -> interpret commands x (y+1)
                                                             "down"   -> interpret commands x (y-1)
                                                             "left"   -> interpret commands (x-1) y
                                                             "right"  -> interpret commands (x+1) y
                                                             "printX" -> (show x):(interpret commands x y)
                                                             "printY" -> (show y):(interpret commands x y)
                                                             _        -> interpret commands x y

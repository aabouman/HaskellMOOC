...
allValues f (Node a t1 t2) = f a && allValues f t1 && allValues f t2
...
isOrdered (Node a t1 t2) = orderLeft t1 a && orderRight a t2 && isOrdered t1 && isOrdered t2
  where orderLeft t1 b = case t1 of Empty -> True
                                    Node a _ _ -> a < b
        orderRight a t2 = case t2 of Empty -> True
                                     Node b _ _ -> a < b
...
walk steps t = case walkHelper steps t of Empty -> Nothing
                                          Node a _ _ -> Just a
  where
    walkHelper _ Empty = Empty
    walkHelper [] t = t
    walkHelper (StepL:steps) (Node _ t _) = walkHelper steps t
    walkHelper (StepR:steps) (Node _ _ t) = walkHelper steps t
...
set _ v Empty = Empty
set [] v (Node _ t1 t2) = Node v t1 t2
set (StepL:steps) v (Node a t1 t2) = Node a (set steps v t1) t2
set (StepR:steps) v (Node a t1 t2) = Node a t1 (set steps v t2)
...
search v (Node a t1 t2)
  | v == a = Just []
  | otherwise = case (search v t1, search v t2) of (Just a, _)  -> Just (StepL:a)
                                                   (_, Just a)  -> Just (StepR:a)

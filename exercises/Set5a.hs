...
totalPrice entry = case entry of (MkShoppingEntry name cost num) -> cost * fromIntegral num
...
buyOneMore entry = case entry of (MkShoppingEntry name cost num) -> MkShoppingEntry name cost (num + 1)
...
setName name p = case p of (Person age _) -> Person age name

-- setAge does likewise for age
setAge :: Int -> Person -> Person
setAge age p = case p of (Person _ name) -> Person age name
...
rgb (Mix c1 c2) = zipWith (\x y -> 0.5*(x+y)) (rgb c1) (rgb c2)
...
fromList [] = Empty
fromList ((k,v):key_val_list) = Pair k v (fromList key_val_list)
...
toNat n
  | n < 0     = Nothing
  | otherwise = Just (toNatHelper n)
  where toNatHelper 0 = Zero
        toNatHelper n = PlusOne (toNatHelper (n-1))
...
fromBin = fromBinHelper 0
  where fromBinHelper place bin = case bin of End   -> 0
                                              (O b) -> fromBinHelper (place + 1) b
                                              (I b) -> 2^place + fromBinHelper (place + 1) b

toBin :: Int -> Bin
toBin i
  | i == 0    = O End
  | i == 1    = I End
  | otherwise = case mod i 2 of 0 -> O (toBin (div i 2))
                                1 -> I (toBin (div i 2))